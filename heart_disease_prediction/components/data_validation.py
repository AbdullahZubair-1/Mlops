...
def load_schema(schema_path):
    if not os.path.exists(schema_path):
        raise FileNotFoundError(f"The schema file at path {schema_path} does not exist.")
    with open(schema_path, "r") as file:
        schema = yaml.safe_load(file)
    if not isinstance(schema, dict):
        raise ValueError(f"The schema file at path {schema_path} is not a valid YAML dictionary.")
    return schema
